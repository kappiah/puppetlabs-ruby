      ...
      case $ruby_package {
        ...
        default:{
          case $version {
            ...
            /^2\.4(\..*)*$/:{
              $real_ruby_package  = "${ruby::params::ruby_package}2.4"
              if ! $suppress_warnings and versioncmp($::operatingsystemrelease, '16.04') < 0 {
                warning('Packages for Ruby 2.4 are not available from default repositories.')
              }
            }
            default: {
              $real_ruby_package  = $ruby_package
      ...
      'Debian': {
        if $system_default_bin {
          ...
          case $version {
            ...
            /^2\.4(\..*)*$/:{
              $real_default_bin  = "${ruby::params::ruby_bin_base}2.4"
              if ! $suppress_warnings {
                warning('No binary for Ruby 2.4.x available from default repositories')
              }
            }
            default: {
